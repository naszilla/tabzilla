...
source ../DATASETS_FTTRANSFORMER_RERUN.sh

name=fttransformer-a100-rerun
...
# config file
config_file=/home/shared/tabzilla/TabSurvey/tabzilla_experiment_config_a100.yml
...
do
  ...
  do
    ...
    run_experiment_a100 "${model}" ${DATASETS[j]} ${env} ${instance_base}-${i}-${j} ${experiment_name} ${config_file} >> ${LOG_DIR}/log_${i}_${j}_$(date +"%m%d%y_%H%M%S").txt 2>&1 &
