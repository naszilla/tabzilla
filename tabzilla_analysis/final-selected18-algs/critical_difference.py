...
def graph_ranks(
    ...
    **kwargs,
    ...
    i = 1
    achieved_half = False
    print(f"significant diffs: {nnames}")
    for clq in cliques:
        if len(clq) == 1:
            continue
        print(clq)
...
def draw_cd_diagram(
    ...
    print(f"average ranks: {average_ranks}")
...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    """
    print(f"unique classifiers: {pd.unique(df_perf['classifier_name'])}")
    ...
    print(f"classifiers tested on nb_max_datasets: {classifiers}")
