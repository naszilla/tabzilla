...
class NODE(BaseModelTorch):
        ...
        if args.objective == "regression":
            self.model = nn.Sequential(
                node_lib.DenseBlock(
                    ...
                    choice_function=node_lib.entmax15,
                    bin_function=node_lib.entmoid15,
                ),
                node_lib.Lambda(
                    lambda x: x[..., 0].mean(dim=-1)
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        experiment_name = "{}_{}.{:0>2d}.{:0>2d}_{:0>2d}:{:0>2d}:{:0>2d}".format(
            self.args.dataset, *time.gmtime()[:6]
        )
        ...
        for batch in node_lib.iterate_minibatches(
            data.X_train,
            data.y_train,
            ...
            if (self.args.epochs - self.trainer.step) % self.args.logging_period == 0:
                ...
                if self.args.objective == "regression":
                    loss = self.trainer.evaluate_mse(
                        data.X_valid,
                        data.y_valid,
                        device=self.device,
                        batch_size=self.args.batch_size,
                    ...
                    loss = self.trainer.evaluate_logloss(
                        data.X_valid,
                        data.y_valid,
                        device=self.device,
                        batch_size=self.args.batch_size,
                    ...
                    loss = self.trainer.evaluate_binarylogloss(
                        data.X_valid,
                        data.y_valid,
                        device=self.device,
                        batch_size=self.args.batch_size,
            ...
            if self.trainer.step > best_step_loss + early_stopping:
                print(
                    "BREAK. There is no improvment for {} steps".format(early_stopping)
                )
    ...
    def predict_helper(self, X):
        ...
        with torch.no_grad():
            prediction = process_in_chunks(
                self.model, X_test, batch_size=self.args.val_batch_size
            )
    ...
    def define_trial_parameters(cls, trial, args):
        params = {
            ...
            "total_tree_count": trial.suggest_categorical(
